...
#Load
MODEL='facebook/'+args.model
tokenizer = AutoTokenizer.from_pretrained(MODEL, use_fast=False, cache_dir='cache/')
model = OPTForCausalLM.from_pretrained(MODEL, cache_dir='cache/')
model.eval()
model.cuda()
...
def get_context(eval_path, form, template_id, name, gender):
  eval=pd.read_csv(eval_path)
  template_ids=list(range(0,9))
  template_ids=[t for t in template_ids if t!= template_id]
  #temp_samp=random.sample(template_ids,2)
  #temp_samp2=template_ids+template_ids+temp_samp
  temp_samp=random.sample(template_ids,1)
  temp_samp2=template_ids+temp_samp

  #print(len(temp_samp))

  context=''
  for t in temp_samp:
    df=eval[(eval.template_id==t) & (eval.name != name) & (eval.form==form)]
    df=df.sample()
    df=df.reset_index()
    #print(df)
    genders=gender_codebook[gender_codebook['gender']!=gender]['gender'].tolist()
    gender2=random.sample(genders,1)[0]
    sent=declare_pronouns(gender2, df['sentence'][0])
    #print(sent)
    substitute=gender_codebook[gender_codebook['gender']==gender2][form].tolist()[0]
    if label_to_cap(sent):
     substitute=substitute.capitalize()
    sent=re.sub('{mask_token}',substitute , sent)
...
def evaluate(eval_path, gender):
  ...
  for idx, instance in eval.iterrows():
    ...
    context= get_context(eval_file,form, instance['template_id'], instance['name'], gender)
