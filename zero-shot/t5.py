...
model = T5ForConditionalGeneration.from_pretrained(MODEL, torch_dtype=torch.float16)
model.eval()
model.cuda()
...
def declare_mask(sentence):
	sentence=re.sub('{mask_token}', '<extra_id_0>', sentence)
	return sentence
...
def get_eval_instance(sent_piece, gender, form):
  ...
  sentence=declare_mask(sentence)
...
def evaluate(eval_path, gender):
  ...
  for idx, instance in eval.iterrows():
    ...
    for c in constraints:
        choice='<extra_id_0> '+c+' <extra_id_1>'
        label_ids=tokenizer(choice, return_tensors="pt")["input_ids"].to('cuda')
