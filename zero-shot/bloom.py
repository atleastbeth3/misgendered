...
#Load
MODEL='bigscience/'+args.model
tokenizer = BloomTokenizerFast.from_pretrained(MODEL, use_fast=False,cache_dir='cache/')
#model = BloomForCausalLM.from_pretrained(MODEL, torch_dtype=torch.float16)
model = BloomForCausalLM.from_pretrained(MODEL, cache_dir='cache/')
model.eval()
model.cuda()
